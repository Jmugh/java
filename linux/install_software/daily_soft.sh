...
install_chrome(){
	...
	if [ -d "crx" ]; then
		echo "install crx for chrome ..."
		google-chrome-stable --no-message-box --no-first-run --no-default-browser-check --disable-default-apps --load-extension=./crx
		echo "v2ray is not checked, please check it..."
	else
		echo "crx/ directory is not exist, please download crx for 'switchyOmega' and untract it to crx..."
	fi
...
read -p "是否安装vim?y/n:" to_install_vim
if [ $to_install_vim == "y" ];then
	install_vim
fi
read -p "是否安装chrome浏览器?y/n:" to_install_chrome
if [ $to_install_chrome == "y" ];then
	install_chrome
fi


read -p "是否安装jdk8?y/n:" to_install_jdk8
if [ $to_install_jdk8 == "y" ];then
	install_jdk8
fi

read -p "是否安装docker?y/n:" to_install_docker
if [ $to_install_docker == "y" ];then
	install_docker
fi

read -p "是否使用docker安装mysql?y/n:" to_install_mysql
if [ $to_install_mysql == "y" ];then
	install_mysql
fi

read -p "是否使用docker安装redis?y/n:" to_install_redis
if [ $to_install_redis == "y" ];then
	install_redis
fi
