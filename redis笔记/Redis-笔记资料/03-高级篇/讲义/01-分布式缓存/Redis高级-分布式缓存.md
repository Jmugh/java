...
<font color="red">**详细流程：主进程会维护一个页表，通过映射物理内存中的文件，来操作内存。bgsave时，会fork一个子进程，复制主进程的页表，通过页表对内存中数据进行持久化。那么保存时候有程序进行修改数据怎么办呢？会将数据设置成read-only, 需要修改数据时，拷贝数据，再进行写，写完之后如果不是readonly状态 就可以写回原内存**</font>
...
<font color="red">Sentinel基于心跳机制监测服务状态，每隔1秒向集群的每个实例发送ping命令：</font>
...
- <font color="red">**master之间通过ping监测彼此健康状态**</font>
